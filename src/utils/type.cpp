...
bool Typename::HasMember(const std::string &name) const {
  return member_.contains(name);
}
...
bool Typename::HasFunction(const std::string &name) const {
  return function_.contains(name);
}
...
Type Typename::CreateType(std::size_t dim) const {
  return {*this, dim};
}
